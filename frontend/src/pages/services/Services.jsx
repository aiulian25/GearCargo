...
export default function Services() {
    ...
    const fetchEntries = async () => {
      ...
      try {
        const response = selectedVehicle === 'all' 
          ...
          : await serviceApi.getByVehicle(selectedVehicle)
        setEntries(response.data.entries || [])
      } catch (error) {
        console.error('Failed to fetch service entries:', error)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const getServiceIcon = (type) => {
    const icons = {
      ...
      brake_service: 'do_not_disturb_on',
      air_filter: 'air',
      full_service: 'build',
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-lg font-semibold">Service Log</h1>
          ...
          {entries.map(entry => (
            ...
            >
              <div className="w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center flex-shrink-0">
                <span className="material-icons-outlined icon-sm text-blue-500">
