...
export default function VehicleChat() {
  ...
  useEffect(() => {
    vehicleApi.getById(id).then((r) => setVehicle(r.data)).catch(() => {})
  }, [id])
  ...
  // Seed the branded greeting as a UI-only assistant message (§13.1/§13.3): it
  // is never sent to the model, shows instantly, and carries the logo avatar.
  // Scope-aligned wording (no "ask me anything"); gracefully degrades when the
  // name or vehicle make/model are missing. Re-seeds on language/name change
  // while the conversation hasn't started.
  useEffect(() => {
    if (!vehicle || hasUserMessage) return
    const firstName = (user?.name || '').trim().split(/\s+/)[0]
    // Vehicle descriptor "{year} {make} {model}" — drop any missing parts.
  ...
  const send = useCallback(async (text) => {
    ...
    setLoading(true)
    try {
      const res = await vehicleApi.chat(id, question, language)
  ...
  }, [id, input, language, loading, errorTextForCode])
  ...
  const suggestions = [
    t('chat.suggestService') || 'When is my next service due?',
    t('chat.suggestFuelYear') || 'How much did I spend on fuel last year?',
    t('chat.suggestTotal') || 'What have I spent on this vehicle in total?',
  ]
    ...
    <div className="flex flex-col min-h-[calc(100vh-3.5rem)] max-w-2xl mx-auto">
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-3" role="log" aria-live="polite" aria-label={t('chat.title') || 'Vehicle Assistant'}>
        {messages.map((m, i) => {
          ...
          // On a guardrail refusal, show the authoritative localized line so the
          // wording is always exact regardless of what the model emitted.
          const text = isAssistant && m.refused ? (t('chat.refusal') || m.text) : m.text
          // §13.4 — assistant AND error bubbles carry the app-logo avatar
          // (the assistant reporting an error is still "speaking"), alt = name.
